...
9. 缓存指令增强：清空缓存需用户二次确认，防止误操作
10. Token用量监控：累计超限主动弹窗警告
...
Q：输入长JD或简历被守卫拦截？
A：已更新 guard.py，业务关键词优先于闲聊词，长文本直接放行。

Q：解析结果大量显示「信息不明确」？
A：粘贴完整规范简历原文，或调整 parser 内部 Prompt。

Q：Token 消耗过快？
A：不要一次性粘贴大量代码；定期执行清空缓存重置统计。
